...
transactions.post('/', async (c) => {
  try {
    ...
    // Backward compatibility: some legacy DBs only allow ('xlsx','pdf') in ingested_files.source_type.
    // In that case INSERT OR IGNORE with source_type='manual' gets ignored, which later breaks FK on transactions.
    const sourceFileRow = await c.env.DB
      .prepare('SELECT 1 FROM ingested_files WHERE file_hash = ? LIMIT 1')
      .bind(sourceFileHash)
      .first();
    if (!sourceFileRow) {
      await c.env.DB.prepare(`
      ...
      `).bind(
        ...
        'Manual entry',
        now,
        JSON.stringify({ source: 'manual', compat_source_type: 'xlsx', created_at: now }),
        scopeUserId
      ).run();
    }

    const ensuredSourceFileRow = await c.env.DB
      .prepare('SELECT 1 FROM ingested_files WHERE file_hash = ? LIMIT 1')
      .bind(sourceFileHash)
      .first();
    if (!ensuredSourceFileRow) {
      return c.json({ error: 'Failed to prepare manual source record' }, 500);
    }
