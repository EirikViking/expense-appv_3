-- Migration 002: Analytics data model
-- Categories, tags, rules, budgets, recurring detection, and transaction enrichment
...
-- Additional indexes on transactions table for analytics queries
...
INSERT OR IGNORE INTO categories (id, name, parent_id, color, icon, sort_order) VALUES
  ('cat_food', 'Food & Dining', NULL, '#ef4444', 'utensils', 1),
  ('cat_food_groceries', 'Groceries', 'cat_food', '#f87171', 'shopping-cart', 1),
  ('cat_food_restaurants', 'Restaurants', 'cat_food', '#fca5a5', 'store', 2),
  ('cat_food_coffee', 'Coffee & Snacks', 'cat_food', '#fecaca', 'coffee', 3),
  ('cat_transport', 'Transportation', NULL, '#3b82f6', 'car', 2),
  ('cat_transport_fuel', 'Fuel', 'cat_transport', '#60a5fa', 'fuel', 1),
  ('cat_transport_public', 'Public Transit', 'cat_transport', '#93c5fd', 'train', 2),
  ('cat_transport_parking', 'Parking', 'cat_transport', '#bfdbfe', 'parking', 3),
  ('cat_shopping', 'Shopping', NULL, '#8b5cf6', 'shopping-bag', 3),
  ('cat_shopping_clothing', 'Clothing', 'cat_shopping', '#a78bfa', 'shirt', 1),
  ('cat_shopping_electronics', 'Electronics', 'cat_shopping', '#c4b5fd', 'laptop', 2),
  ('cat_shopping_home', 'Home & Garden', 'cat_shopping', '#ddd6fe', 'home', 3),
  ('cat_entertainment', 'Entertainment', NULL, '#ec4899', 'film', 4),
  ('cat_entertainment_streaming', 'Streaming Services', 'cat_entertainment', '#f472b6', 'tv', 1),
  ('cat_entertainment_games', 'Games', 'cat_entertainment', '#f9a8d4', 'gamepad', 2),
  ('cat_entertainment_events', 'Events & Activities', 'cat_entertainment', '#fbcfe8', 'ticket', 3),
  ('cat_bills', 'Bills & Utilities', NULL, '#f59e0b', 'file-text', 5),
  ('cat_bills_electricity', 'Electricity', 'cat_bills', '#fbbf24', 'zap', 1),
  ('cat_bills_internet', 'Internet & Phone', 'cat_bills', '#fcd34d', 'wifi', 2),
  ('cat_bills_insurance', 'Insurance', 'cat_bills', '#fde68a', 'shield', 3),
  ('cat_health', 'Health & Wellness', NULL, '#10b981', 'heart', 6),
  ('cat_health_pharmacy', 'Pharmacy', 'cat_health', '#34d399', 'pill', 1),
  ('cat_health_fitness', 'Fitness', 'cat_health', '#6ee7b7', 'dumbbell', 2),
  ('cat_health_medical', 'Medical', 'cat_health', '#a7f3d0', 'stethoscope', 3),
  ('cat_travel', 'Travel', NULL, '#06b6d4', 'plane', 7),
  ('cat_travel_lodging', 'Lodging', 'cat_travel', '#22d3ee', 'bed', 1),
  ('cat_travel_flights', 'Flights', 'cat_travel', '#67e8f9', 'plane-takeoff', 2),
  ('cat_income', 'Income', NULL, '#22c55e', 'wallet', 8),
  ('cat_income_salary', 'Salary', 'cat_income', '#4ade80', 'banknote', 1),
  ('cat_income_refund', 'Refunds', 'cat_income', '#86efac', 'rotate-ccw', 2),
  ('cat_transfer', 'Transfers', NULL, '#6b7280', 'repeat', 9),
  ('cat_other', 'Other', NULL, '#9ca3af', 'more-horizontal', 10);
